...
import (
	...
	alertinggateway "odfe-cli/gateway/alerting/mocks"
	"odfe-cli/mapper"
	"os"
	"path/filepath"
	"testing"
...
func getCreateMonitorRequest() entity.CreateMonitorRequest {
	return entity.CreateMonitorRequest{
...
func getCreateMonitor() *entity.CreateMonitor {
	return &entity.CreateMonitor{
		Name:        "testdata-detector",
		Description: "Test detector",
		TimeField:   "timestamp",
		Index:       []string{"order*"},
...
func TestController_StartDetector(t *testing.T) {
	t.Run("start empty detector", func(t *testing.T) {
		...
		mockAlertingGateway := alertinggateway.NewMockGateway(mockCtrl)
		mockESController := esmockctrl.NewMockController(mockCtrl)
		ctx := context.Background()
		ctrl := New(os.Stdin, mockESController, mockAlertingGateway)
	...
	t.Run("start detector gateway failed", func(t *testing.T) {
		...
		defer mockCtrl.Finish()
		ctx := context.Background()
		mockADGateway := alertinggateway.NewMockGateway(mockCtrl)
...
func TestController_CreateMonitors(t *testing.T) {
	t.Run("gateway failed", func(t *testing.T) {
		mockCtrl := gomock.NewController(t)
		defer mockCtrl.Finish()
		ctx := context.Background()
		r := getCreateMonitorRequest()
		mockAlertingGateway := alertinggateway.NewMockGateway(mockCtrl)
		mockAlertingGateway.EXPECT().CreateMonitor(ctx, getCreateMonitor()).Return(nil, errors.New("failed to connect"))
		mockESController := esmockctrl.NewMockController(mockCtrl)
		ctrl := New(os.Stdin, mockESController, mockAlertingGateway)
		_, err := ctrl.CreateMonitors(ctx, r)
	...
	t.Run("entity failed to create", func(t *testing.T) {
		...
		r := getCreateMonitorRequest()
		mockADGateway := alertinggateway.NewMockGateway(mockCtrl)
		mockADGateway.EXPECT().CreateMonitor(ctx, getCreateMonitor()).Return(nil, errors.New(string(helperLoadBytes(t, "create_failed_response.json"))))
		mockESController := esmockctrl.NewMockController(mockCtrl)
		ctrl := New(os.Stdin, mockESController, mockADGateway)
		_, err := ctrl.CreateMonitors(ctx, r)
	...
	t.Run("entity succeeded without starting", func(t *testing.T) {
		...
		r.Start = false
		mockADGateway := alertinggateway.NewMockGateway(mockCtrl)
		mockADGateway.EXPECT().CreateMonitor(ctx, getCreateMonitor()).Return(helperLoadBytes(t, "create_response.json"), nil)
		mockESController := esmockctrl.NewMockController(mockCtrl)
		ctrl := New(os.Stdin, mockESController, mockADGateway)
		detectorID, err := ctrl.CreateMonitors(ctx, r)
		assert.NoError(t, err)
...
func TestController_CreateMultiEntityMonitors(t *testing.T) {
	t.Run("create one detector", func(t *testing.T) {
		...
		r := getCreateMonitorRequest()
		mockADGateway := alertinggateway.NewMockGateway(mockCtrl)
		gatewayRequest := getCreateMonitor()
		gatewayRequest.Name = gatewayRequest.Name + "-" + "localhost"
		gatewayRequest.Filter = getFinalFilter(getRawFilter())
		mockADGateway.EXPECT().CreateMonitor(ctx, gatewayRequest).Return(helperLoadBytes(t, "create_response.json"), nil)
		mockADGateway.EXPECT().StartDetector(ctx, mockDetectorID).Return(nil)
		mockESController := esmockctrl.NewMockController(mockCtrl)
		...
		ctrl := New(os.Stdin, mockESController, mockADGateway)
		detectorID, err := ctrl.CreateMultiEntityMonitors(ctx, r, false, false)
		assert.NoError(t, err)
		assert.NotNil(t, detectorID)
	...
	t.Run("create detector failed due to second detector", func(t *testing.T) {
		...
		gatewayRequest.Filter = getFinalFilter(getRawFilter())
		mockADGateway.EXPECT().CreateMonitor(ctx, gatewayRequest).Return(helperLoadBytes(t, "create_response.json"), nil)
		mockADGateway.EXPECT().CreateMonitor(ctx, gatewayRequest).Return(nil, errors.New(string(helperLoadBytes(t, "create_failed_response.json"))))
		mockADGateway.EXPECT().StartDetector(ctx, mockDetectorID).Return(nil)
		...
		ctrl := New(os.Stdin, mockESController, mockADGateway)
		_, err := ctrl.CreateMultiEntityMonitors(ctx, r, false, false)
		assert.EqualError(t, err, "Cannot create anomaly detector with name [testdata-detector] as it's already used by detector [wR_1XXMBs3q1IVz33Sk-]")
	})
	t.Run("create one detector with no filter", func(t *testing.T) {
		...
		stdin.Write([]byte("yes\n"))
		ctrl := New(&stdin, mockESController, mockADGateway)
		detectorID, err := ctrl.CreateMultiEntityMonitors(ctx, r, true, false)
		assert.NoError(t, err)
		assert.NotNil(t, detectorID)
		assert.EqualValues(t, gatewayRequest.Name, detectorID[0])
	})
...
func TestController_UpdateMonitor(t *testing.T) {
	input := entity.UpdateMonitorUserInput{
	...
	t.Run("update detector without ID", func(t *testing.T) {
		...
		ctx := context.Background()
		ctrl := New(os.Stdin, mockESController, mockADGateway)
		err := ctrl.UpdateMonitor(ctx, invalidInput, true, true)
		assert.EqualError(t, err, "detector Id cannot be empty")
	})
	t.Run("stale detector update failure", func(t *testing.T) {
		...
		stdin.Write([]byte("yes\n"))
		ctrl := New(&stdin, mockESController, mockADGateway)
		err := ctrl.UpdateMonitor(ctx, staleDetector, false, true)
	...
	t.Run("don't update if user says no", func(t *testing.T) {
		...
		stdin.Write([]byte("no\n"))
		ctrl := New(&stdin, mockESController, mockADGateway)
		err := ctrl.UpdateMonitor(ctx, input, false, true)
		assert.NoError(t, err)
	})
	t.Run("stop detector gateway failed", func(t *testing.T) {
		mockCtrl := gomock.NewController(t)
		...
		stdin.Write([]byte("yes\n"))
		ctrl := New(&stdin, mockESController, mockADGateway)
		err := ctrl.UpdateMonitor(ctx, input, true, true)
	...
	t.Run("get detector gateway failed", func(t *testing.T) {
		...
		mockESController := esmockctrl.NewMockController(mockCtrl)
		ctrl := New(os.Stdin, mockESController, mockADGateway)
		err := ctrl.UpdateMonitor(ctx, input, false, false)
	...
	t.Run("update detector gateway failed", func(t *testing.T) {
		...
		mockADGateway := alertinggateway.NewMockGateway(mockCtrl)
		mockADGateway.EXPECT().GetDetector(ctx, "m4ccEnIBTXsGi3mvMt9p").Return(helperLoadBytes(t, "get_response.json"), nil)
		mockADGateway.EXPECT().UpdateMonitor(ctx, "m4ccEnIBTXsGi3mvMt9p", &request).Return(errors.New("failed to update"))
		mockESController := esmockctrl.NewMockController(mockCtrl)
		var stdin bytes.Buffer
		stdin.Write([]byte("yes\n"))
		ctrl := New(&stdin, mockESController, mockADGateway)
	...
	t.Run("start detector gateway failed", func(t *testing.T) {
		...
		mockADGateway := alertinggateway.NewMockGateway(mockCtrl)
		mockADGateway.EXPECT().GetDetector(ctx, "m4ccEnIBTXsGi3mvMt9p").Return(helperLoadBytes(t, "get_response.json"), nil)
		mockADGateway.EXPECT().UpdateMonitor(ctx, "m4ccEnIBTXsGi3mvMt9p", &request).Return(nil)
	...
	t.Run("force detector update success", func(t *testing.T) {
		...
		stdin.Write([]byte("yes\n"))
		ctrl := New(&stdin, mockESController, mockADGateway)
		err := ctrl.UpdateMonitor(ctx, input, true, false)
		assert.NoError(t, err)
	})
